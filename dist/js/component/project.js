  ...
  for (const data of projectData) {
    const getData = `
      <a id="${data.id}" href="${data.url}" target="_blank" class="group hover:scale-95 w-full aspect-[4/3] duration-700 ease-in-out rounded-t-xl overflow-hidden relative">
          ...
          <h3 class="bg-gradient-to-t from-hitam-100 to-hitam-100 text-base duration-700 ease-in-out md:text-2xl w-full font-normal group-hover:font-medium p-3 md:p-8 absolute bottom-0 left-0">${data.title}</h3>
